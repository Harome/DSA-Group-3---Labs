        ...
        if selected_size in data_size_mapping:
            ...
            if not array_str or not target_str or not search_type:
                return render_template(
                    ...
                    error="Invalid input. Ensure all fields are filled.",
                )
        else:
            return render_template(
                "Website_html/works.html", error="Invalid data size."
            )
...
def triangle_aaron():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            error = (
                "Invalid input. Please enter valid integers for the base and height."
            )
...
def merge_sorted_linked_lists(list1, list2):
    ...
    while list1 is not None and list2 is not None:
        ...
        if isinstance(list1.value, (int, float)) and isinstance(
            list2.value, (int, float)
        ):
...
def marga_push():
    info = request.form.get(
        "stackInfo", ""
    )  # assuming 'stackInfo' is the name of the form field
    if info:
...
def remove():
    ...
    return render_template(
        "Website_html/qqq.html", queue=queue_list, removed_data=removed_data
    )
...
def pathf():
    ...
    if request.method == "POST":
        ...
        shortest_path_length, shortest_path_stations = mrt_graph.shortest_path(
